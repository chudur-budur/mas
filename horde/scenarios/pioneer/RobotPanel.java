...
public class RobotPanel extends JPanel implements MacroObserver
{
	...
	public RobotPanel(final JFrame mainWindow)
	{
					...
					public void run() {
						...
						for (int i=0; i < horde.agents.size(); i++)
						{
							...
							if (seeGreen.getValue(a, (Macro)a.getBehavior(), horde) >= 0)
								seeAttack = 1;
							if (((TrainableMacro)a.behavior).getFlag(Macro.FLAG_DONE))
								doneCnt++;
							if (seeBlue.getValue(a, (Macro)a.getBehavior(), horde) >= 0) {
								...
								if (System.currentTimeMillis() - lastSeenBlue[0] <
										SCATTER_TIME)
									seeScatter = 1;
							}
						}
						if (doneCnt > 0) oneDone = 1;
						if (doneCnt == horde.agents.size()) allDone = 1;
		...
		final JButton showTreeButton = new JButton("Show models");
		showTreeButton.setToolTipText("Show models");
		...
		saveTreesButton.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				String fileName = (String) JOptionPane.showInputDialog(null,
				                  "Enter a filename for the behavior");
				...
				if (fileName != null)
				{
					...
					// save it!!!
					horde.save(fileName, KeyStroke.getKeyStroke('s'), 0);
				}
		...
		training.addItemListener(new ItemListener()
		{
			public void itemStateChanged(ItemEvent evt)
			{
				if (training.isSelected())
				{
					...
					horde.getTrainingMacro().learnPolicy(horde, true);
				}
				else
				{
					System.out.println("DONE TRAINING");
					horde.getTrainingMacro().learnPolicy(horde, false);
				...
				ActionListener listener = new ActionListener()
				{
					public void actionPerformed(ActionEvent e)
					{
						if (!training.isSelected())
						{
							...
							horde.setRecording(training.isSelected());
						}
