...
public class Utilities
{
	...
	*/
	...
	public static double relativeAngle(Double2D currentLocation, double currentOrientation, Double2D target)
	{
		...
		Utilities.count[1]++;
		double rel_angle =  Utilities.fast_atan2((double)(target.y - currentLocation.y), (double)(target.x - currentLocation.x))
		                    // Math.atan2(target.y - currentLocation.y, target.x - currentLocation.x)
		                    - currentOrientation;

		// normalize angle
		//rel_angle = Utilities.fast_atan2((double)(Math.sin(rel_angle)), (double)(Math.cos(rel_angle)));
	...
	*/
	...
	public static Double2D closestPointOnCircle(double x1, double y1, double radius, double x2, double y2)
	{
		...
		Utilities.count[2]++;
		double angle = Utilities.fast_atan2((double)(y2 - y1), (double)(x2 - x1));
	...
	*/
	...
	/** Highly inaccurate atan */



	private static final int           SIZE                 = 4096;  // was originally 1024.  We can make it bigger still
	...
	public static final double fast_atan2(double y, double x)
	{
		if (x >= 0)
		{
			if (y >= 0)
