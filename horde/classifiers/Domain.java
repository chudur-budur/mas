...
public class Domain implements java.io.Serializable, Cloneable
{
	...
	public Object clone()
	{
		try
		{
			...
			for(int i = 0 ; i < f.values.length; i++)
			{
				if (values[i] == null)
				{
					f.values[i] = null;    // no array for some reason
					continue;
				}
	...
	public Domain(String name, String[] a, String[][] v, String[] c, int[] type
	...
	public static Domain loadDomain(String aFileName, MersenneTwisterFast random)
	{
			...
			for (int lineNumber = 0; scanner.hasNextLine(); lineNumber++)
			{
				...
				else if (line.startsWith("}"))
				{
					if (state != DONE)
						...
						throw new RuntimeException("Parse Error in line "
						                           + lineNumber + " of " + aFileName +
						                           "\n No Section to Close");
					continue;
				}
				else if (state == DONE &&
				...
				{
					// first read category
					// -- there must be one or we would have had a blank line
					...
					if (!lineScanner.hasNext()) // no values at all
						throw new RuntimeException("Parse Error in line "
						                           + lineNumber + " of " + aFileName
						                           + "\n Categorical Attribute has no values "
						                           + val);
				...
				else if (state == TOROIDAL)
				{
					...
					*/
					...
					throw new RuntimeException("Parse Error in line " + lineNumber
					                           + " of " + aFileName
		...
		return new Domain(aFileName.substring(0, aFileName.length() - 3),
		                  attributes, values, classes, types /*, random */);
	}
	...
	void readAttributes(Scanner scanner)
	{
		...
		while(true)
		{
			...
			else generateTokenError(scanner, new String[] {"categorical", "continuous",
				                        "toroidal"
				                                              });
		}
		// convert arraylists
