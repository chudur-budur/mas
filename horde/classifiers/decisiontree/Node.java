...
public abstract class Node implements java.io.Serializable, Cloneable
{
	...
	*/
	...
	protected static Node learn(Domain dom, Example[] examples, int[] attributes, Leaf defaultLeaf, int maxDepth, int depth, MersenneTwisterFast random)
	{
		...
		else
		{
			...
			case Domain.TYPE_CATEGORICAL:
				return CategoricalNode.provideNode(dom, bestAttribute, examples, attributes, maxDepth, depth, new Leaf(dom, examples, examples), random);
			case Domain.TYPE_CONTINUOUS:
				return ContinuousNode.provideNode(dom, bestAttribute, examples, attributes, maxDepth, depth, new Leaf(dom, examples, examples), random);
			case Domain.TYPE_TOROIDAL:
				return ToroidalNode.provideNode(dom, bestAttribute, examples, attributes, maxDepth, depth, new Leaf(dom, examples, examples), random);
			default:
				throw new RuntimeException("No such node type " + dom.type[bestAttribute]);  // better not happen!
			}
		}
	}
	...
	{
		return Math.log(val) * invertlog2;    // Ask Sean: log base 2 required?  -- Not sure, probabably not -- Sean
	}


	/** Computes the information among the classes in the given examples. */
	public static double information(Domain dom, Example[] examples)
	{
		return information (dom, examples, 0, examples.length, true);
	}
	...
	*/
	...
	public static int chooseAttribute(Domain dom, int[] attributes, Example[] examples, MersenneTwisterFast random)
	{
		...
		for(int i=0; i<attributes.length; i++)
		{
			...
			if(gains[i] < averageGain) continue; // below average
			...
			*/
			...
			//System.out.println(value + "\t" + max_value + "\t" + count);
	...
	public void updateUniqueInteger()
	{
		uniqueInteger = privateCounter++;
	}
	...
	public static void resetUniqueInteger()
	{
		privateCounter = 0;
	}
	...
	public Node(Scanner scanner, Domain domain)
	{
		dom = domain;
	}
	...
	protected  static String token(Scanner scanner)
	{
		return token(scanner, null);
	}
	...
	{
		generateTokenError(scanner, new String[] { expect });
	}
	protected  static void generateTokenError(Scanner scanner, String expect1, String expect2)
	{
		generateTokenError(scanner, new String[] { expect1,  expect2 });
	}
	protected  static void generateTokenError(Scanner scanner, String[] expect)
	{
