...
public abstract class Target implements Serializable, Cloneable
{
	...
	static
	{
		...
		Scanner scanner = new Scanner(Horde.class.getResourceAsStream(Horde.BASIC_TARGETS_LOCATION));
		while(scanner.hasNextLine())
		{
			...
			{
				System.err.println("Couldn't find Target: " + s);
			}
		}
		scanner.close();
	...
	*/
	...
	public static Target[] provideAllTargets(TrainableMacro macro)
	{
		if(!talked)
		{
			System.err.println("--- Target.provideAllTargets() :"
			                   + " providing all targets.");
			talked = true ;
		}
		...
		for(int i = 0; i < basicTargetClasses.length; i++)
			try
			{
				targets[i+tlen] = (Target)(basicTargetClasses[i].newInstance());
			}
			catch (Exception e)
			{
				e.printStackTrace();
